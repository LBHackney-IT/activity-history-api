...
namespace ActivityHistoryApi.V1.Controllers
{
    ...
    public class ActivityHistoryApiController : BaseController
    {
        ...
        private readonly IGetByTargetIdAndActivityTypeUseCase _getByTargetIdAndActivityTypeUseCase;

        public ActivityHistoryApiController(IGetByTargetIdUseCase getByTargetIdUseCase, IGetByTargetIdAndActivityTypeUseCase getByTargetIdAndActivityTypeUseCase)
        {
            _getByTargetIdUseCase = getByTargetIdUseCase;
            _getByTargetIdAndActivityTypeUseCase = getByTargetIdAndActivityTypeUseCase;
        }
        ...
        /// </summary>
        /// <response code="200">Returns the list of activity history for the supplied target id.</response>
        ...
        /// <summary>
        /// Retrieves activity history for the supplied targetId and activity type.
        /// Essentially like the other endpoint but with one extra filter
        /// If a pagination token is provided (returned from a previous call where the results set > 10)
        /// then the query will continue from the last record returned in the preivious query.
        /// </summary>
        /// <response code="200">Returns the list of activity history for the supplied target id.</response>
        ...
        [HttpGet]
        [LogCall(LogLevel.Information)]
        public async Task<IActionResult> GetByTargetIdAndActivityTypeAsync([FromQuery] GetActivityHistoryByTargetIdAndActivityTypeQuery query)
        {
            var response = await _getByTargetIdAndActivityTypeUseCase.ExecuteAsync(query).ConfigureAwait(false);
            if ((null == response) || !response.Results.Any()) return NotFound(query.TargetId);
