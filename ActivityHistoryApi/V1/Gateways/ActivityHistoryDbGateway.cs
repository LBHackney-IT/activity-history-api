...
namespace ActivityHistoryApi.V1.Gateways
{
    public class ActivityHistoryDbGateway : IActivityHistoryGateway
    {
        ...
        private const string ACTIVITYTYPE = "newData._activityType";
        ...
        [LogCall]
        public async Task<PagedResult<ActivityHistoryEntity>> GetByTargetIdAndActivityTypeAsync(GetActivityHistoryByTargetIdAndActivityTypeQuery query)
        {
            int pageSize = query.PageSize.HasValue ? query.PageSize.Value : MAX_RESULTS;
            var activityHistoryDB = new List<ActivityHistoryDB>();
            var table = _dynamoDbContext.GetTargetTable<ActivityHistoryDB>();
            var filter = new QueryFilter(TARGETID, QueryOperator.Equal, query.TargetId);
            filter.AddCondition(ACTIVITYTYPE, QueryOperator.Equal, query.ActivityType);
            var queryConfig = new QueryOperationConfig
            {
                IndexName = GETACTIVITIESBYTARGETIDINDEX,
                BackwardSearch = true,
